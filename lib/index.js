...
_setJshintResult = function(file, shim, relName) {
  var actualGlobals, jshglobals, result, text;
  ...
  if (_(jsh.JSHINT.data().implieds).pluck('name').any(function(item) {
    return item === "window";
  })) {
    _(jsh.JSHINT.data().implieds).pluck('name').concat(_(amdify.globals).keys().value()).difference(envVars).concat(_.flatten([shim["export"] || []])).each(function(implied) {
      var glreg;
  ...
  if ((jshglobals = _(jsh.JSHINT.data().globals).filter(function(item) {
    return item !== 'undefined';
  }).value()).length) {
    actualGlobals = _(text.match(new RegExp("\\S[\\n\\s]+?(" + jshglobals.join('|') + ")[\\s\\n]*?=", 'g'))).filter(function(exp) {
      return exp.indexOf('.') !== 1;
    }).uniq().map(function(exp) {
      return exp.replace(/\S[\n\s]+?/g, '').replace(/[\s\n]*?=/g, '');
    }).value();
  } else {
    actualGlobals = [];
  }
  file.exports = file.shim["export"] ? _.flatten([file.shim["export"]]) : actualGlobals;
...
_analizeFiles = function(mimosaConfig, options, next) {
  ...
  _(options.files).filter(function(file) {
    ...
    maybe = _([amdify.includePaths]).flatten().map(function(include) {
      ...
      if (!_(relative.split(path.sep)).filter(function(item) {
        return item !== "..";
      }).value().length) {
...
_wrap = function(file, fileAnalysis) {
